...
export const validatePassword = (password: string): boolean => {
  return password.length >= VALIDATION_RULES.PASSWORD.MIN_LENGTH && VALIDATION_RULES.PASSWORD.PATTERN.test(password);
};
...
export const validateStudyName = (name: string): boolean => {
  return name.length >= VALIDATION_RULES.STUDY_NAME.MIN_LENGTH && name.length <= VALIDATION_RULES.STUDY_NAME.MAX_LENGTH;
};
